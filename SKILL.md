...
```
You are running a scheduled Tableau Data Quality Sentinel scan. This is an automated run with no user present.

STEP 1 — LOAD METHODOLOGY:
Read the skill file at this path to understand the full scan methodology:
C:\Users\adam.mico\AppData\Local\Claude-3p\local-agent-mode-sessions\skills-plugin\00000000-0000-4000-8000-000000000001\02c168ca-fcfb-45b7-9a8c-1d92a44e49b3\skills\tableau-data-quality-sentinel\SKILL.md

STEP 2 — LOAD PRIOR STATE:
Read memory files for prior scan results (look for dq-metadata-cache and dq-scan-latest in the memory directory).

STEP 3 — EXECUTE:
Run a DQ scan following the methodology in the SKILL.md:
- Scope: [scope from config]
- Mode: Changed-Only (compare updatedAt against cached timestamps, skip unchanged)
- Format: Alert-style (only report problems and score changes)

STEP 4 — COMPARE:
Compare results to prior scan in memory. Only surface:
...
- Findings that were resolved since last scan

STEP 5 — SAVE TO MEMORY:
Write updated scan results to memory (dq-metadata-cache and dq-scan-latest) so the next scheduled run has a baseline.

STEP 6 — REPORT:
If nothing changed or all scores are stable: "DQ Sentinel: All clear. No score changes since [last scan date]."
If issues found: list them with severity, datasource name, and recommended action.
```

### Scheduled Run Protocol

When this skill is invoked by a scheduled task (no user present, fresh session):

1. **The session has no conversation history.** It must read this SKILL.md file to understand methodology.
2. **The Tableau MCP tools are available** (configured at app level, not per-session).
3. **Memory persists across sessions.** Prior scan results stored in memory are readable and writable.
4. **The session must commit results to memory** after every scan so future sessions (scheduled or interactive) can compute deltas.
5. **No user interaction is possible.** Auto-infer all decisions. Use prior config from memory. Never ask questions.
6. **Output is the notification.** Keep it brief (alert-style). The user sees it as a notification, not a conversation.
